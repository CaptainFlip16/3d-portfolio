...
export default class Car
{
    ...
    setBackLights()
    {
        ...
        this.time.on('tick', () =>
        {
            const isBraking = this.physics.controls.actions.brake
            const isReversing = this.physics.controls.actions.down

            this.backLightsBrake.material.opacity = isBraking ? 1 : 0.5
            this.backLightsReverse.material.opacity = isReversing ? 1 : 0.5

            if(this.brakeLightPoint)
            {
                this.brakeLightPoint.intensity = isBraking ? 4 : 0.8
            }
        })
    }
    ...
    setCyberLights()
    {
        // 1. Cyberpunk Cyan Underglow Light
        this.underglow = new THREE.PointLight(0x00f3ff, 4, 7)
        this.underglow.position.set(0, 0, - 0.1)
        this.chassis.object.add(this.underglow)

        // 2. Headlights / Forward Beams (Cyber Pink Glow)
        this.headlightLeft = new THREE.SpotLight(0xff0055, 6, 12, Math.PI * 0.25, 0.5, 1)
        this.headlightLeft.position.set(- 0.5, 1.2, 0.2)
        this.headlightLeft.target.position.set(- 0.5, 5, 0)

        this.headlightRight = new THREE.SpotLight(0xff0055, 6, 12, Math.PI * 0.25, 0.5, 1)
        this.headlightRight.position.set(0.5, 1.2, 0.2)
        this.headlightRight.target.position.set(0.5, 5, 0)

        this.chassis.object.add(this.headlightLeft)
        this.chassis.object.add(this.headlightLeft.target)
        this.chassis.object.add(this.headlightRight)
        this.chassis.object.add(this.headlightRight.target)

        // 3. Dynamic Tail Light Point Light Projection
        this.brakeLightPoint = new THREE.PointLight(0xff0055, 0.8, 4)
        this.brakeLightPoint.position.set(0, - 1.2, 0.1)
        this.chassis.object.add(this.brakeLightPoint)
    }
