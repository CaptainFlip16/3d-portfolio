...
export default class IntroSection
{
    ...
    setTitles()
    {
        /**
         * CUSTOM TITLE MESHES
         * Update the resource item names below to match the key names defined in your Resources.js manifest.
         */

        // FIRST NAME / PRIMARY TITLE MODEL
        if (this.resources.items.introFirstNameBase) {
            this.objects.add({
                base: this.resources.items.introFirstNameBase.scene,
                collision: this.resources.items.introFirstNameCollision.scene,
                offset: new THREE.Vector3(0, 0, 0),
                rotation: new THREE.Euler(0, 0, 0),
                shadow: { sizeX: 6, sizeY: 1.5, offsetZ: - 0.6, alpha: 0.4 },
                mass: 1.5,
                soundName: 'brick'
            })
        }

        // LAST NAME / SECONDARY TITLE MODEL
        if (this.resources.items.introLastNameBase) {
            this.objects.add({
                base: this.resources.items.introLastNameBase.scene,
                collision: this.resources.items.introLastNameCollision.scene,
                offset: new THREE.Vector3(4.5, 0, 0),
                rotation: new THREE.Euler(0, 0, 0),
                shadow: { sizeX: 6, sizeY: 1.5, offsetZ: - 0.6, alpha: 0.4 },
                mass: 1.5,
                soundName: 'brick'
        ...
        // SUBTITLE LINE 1 (e.g. "CREATIVE" or custom title)
        if (this.resources.items.introCreativeBase) {
            this.objects.add({
        ...
        // SUBTITLE LINE 2 (e.g. "DEVELOPER" or custom title)
        if (this.resources.items.introDevBase) {
            this.objects.add({
                base: this.resources.items.introDevBase.scene,
                collision: this.resources.items.introDevCollision.scene,
                offset: new THREE.Vector3(3.5, -2, 0),
                rotation: new THREE.Euler(0, 0, 0),
    ...
    setAnimations()
    {
        // Subtle Holographic Pulse Animation
        this.time.on('tick', () =>
        {
            const pulse = (Math.sin(this.time.elapsed * 0.003) * 0.15) + 0.85

            if(this.instructions.arrows.label.material)
            {
                this.instructions.arrows.label.material.color.setHSL(0.52, 1, 0.5 * pulse)
            }

            if(this.otherInstructions && this.otherInstructions.label.material)
            {
                this.otherInstructions.label.material.color.setHSL(0.52, 1, 0.5 * pulse)
            }
        })
    }
}